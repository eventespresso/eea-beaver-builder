	...
	<div class="espresso_events_table">
		...
		if ($settings->month != ''){
			...
			echo '<div class="events-table">';
		    	echo do_shortcode( '[ESPRESSO_EVENTS_TABLE_TEMPLATE footable='.$settings->footable.' category_filter='.$settings->category_filter.' table_sort='.$settings->table_sort.' table_search='.$settings->table_search.' table_paging = '.$settings->table_paging.' show_expired='.$settings->show_expired.' limit='.$settings->limit.' '.$month.']' );
		    echo '</div>';
